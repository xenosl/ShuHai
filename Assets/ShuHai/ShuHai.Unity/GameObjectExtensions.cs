...
namespace ShuHai.Unity
{
    public static class GameObjectExtensions
    {
        ...
        private static Component EnsureComponentImpl(this GameObject self, Type type)
        {
            var c = self.GetComponent(type);
            if (!c)
                UnityThrows.MissingComponent(type, self);
            return c;
        }
        ...
        private static Component EnsureComponentInParentImpl(this GameObject self, Type type)
        {
        ...
        private static Component GetOrAddComponentImpl(this GameObject self, Type type)
        {
            var c = self.GetComponent(type);
            if (!c)
        ...
        public static T EnsureComponentInChild<T>(this GameObject self, string path) where T : Component
        {
            ...
            if (!c)
                UnityThrows.MissingComponent<T>(self);
            return c;
        }
